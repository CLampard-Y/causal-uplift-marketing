...
在 uplift/CATE 侧，Phase 2 对比了 S/T/X-Learner，并在 30% held-out test 集上用 Qini 曲线量化 uplift 排序能力。在该评估口径下（`n_test=19,200`），X-Learner 的 Qini Coefficient 为 `1.719097`，优于 S-Learner 的 `1.670222`；T-Learner 为负（`-0.679497`）（见 `notebooks/04_uplift_modeling.ipynb`）。随后，Notebook 04 将 held-out 评估结果落盘到 `data/processed/qini_results.json`，并把三种 learner 在全样本 refit 后的 CATE 向量落盘到 `data/processed/cate_vectors.npz`，供 Phase 3 消费。
...
2) Uplift/CATE（S/T/X-Learner → held-out Qini/AUUC 评估与选优 → full-data refit → 落盘 CATE 向量与评估结果），实现/证据入口：`notebooks/04_uplift_modeling.ipynb`、`src/uplift.py`。
...
下面用真实产物把 Phase 2 的端到端链路串起来：特征表进入 PS 估计与诊断，再进入匹配与平衡性检查并输出 matched ATE；同一份特征表也进入 uplift learner 的 held-out 评估与 Qini 选优。确认上游选型后，Notebook 04 进一步在 full data 上 refit 三种 learners，并分别落盘 `qini_results.json`（保存 held-out 评估/选型元数据）与 `cate_vectors.npz`（保存全样本 CATE 向量），供 Phase 3/4 的分群与 ROI 仿真消费。
...
flowchart LR
  ...
  A --> I[S/T/X-Learner on train/test split<br/>notebooks/04_uplift_modeling.ipynb]
  I --> I1[Figure<br/>outputs/figures/fig_05_cate_distributions.png]
  I --> J[Qini / AUUC on held-out test<br/>src/uplift.compute_qini]
  J --> J1[Figure<br/>outputs/figures/fig_06_qini_curves.png]
  J --> L[Held-out eval + best learner<br/>data/processed/qini_results.json]
  A --> K[Full-data refit S/T/X-Learner<br/>notebooks/04_uplift_modeling.ipynb]
  K --> M[Full-sample CATE vectors<br/>data/processed/cate_vectors.npz]
```
...
- Uplift 侧同时运行 S/T/X-Learner，并先在 held-out test split 上用 Qini coefficient 选优；learner 的基础超参由配置提供：`uplift.n_estimators=100`、`uplift.max_depth=5`、`general.random_state=42`（见 `configs/config.yml`），并在 `notebooks/04_uplift_modeling.ipynb` 中以 `test_size=0.3` 做了 train/test split。当前 run 中 X-Learner 的 Qini Coefficient `1.719097` 最高；Notebook 04 随后将 held-out 评估结果与全样本 refit 的 CATE 向量分别落盘到 `data/processed/qini_results.json` 和 `data/processed/cate_vectors.npz`，供 Phase 3 直接消费。
...
复跑后可额外查看落盘审计产物：`data/processed/psm_match_panel.json`、`data/processed/qini_results.json`（held-out 评估 + `best_learner` 元数据）、`data/processed/cate_vectors.npz`（full-sample refit CATE 向量）。
...
- 本阶段的直接产出是 matched-sample diagnostics、`qini_results.json` 中的 held-out Qini 评估/选型元数据，以及 `cate_vectors.npz` 中的全样本 CATE 向量；这些产物将在 Phase 3 中继续用于分群与 ROI 仿真。
...
在 uplift 建模侧，Phase 2 于 `notebooks/04_uplift_modeling.ipynb` 中对比 S/T/X-Learner，并用 Qini 曲线在 held-out test 集上评估排序能力；在 `n_test=19,200` 的 test 集上，X-Learner 的 Qini Coefficient `1.719097` 表现最佳。Notebook 04 随后把该评估结果落盘到 `data/processed/qini_results.json`，并将三种 learner 在全样本 refit 后的 CATE 向量落盘到 `data/processed/cate_vectors.npz`。
