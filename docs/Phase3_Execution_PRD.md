...
Phase 3 的目标，是把 Phase 2 已落盘的个体级因果增量（CATE）从“模型输出”转成“离线策略建议”。具体做法是：先读取 `data/processed/qini_results.json` 中的 `best_learner` 元数据，并从 `data/processed/cate_vectors.npz` 载入对应的全样本 CATE 向量，再将其划分为四象限人群（Persuadables / Sure Things / Lost Causes / Sleeping Dogs）。在此基础上，本阶段选定的默认离线策略是 **只触达 Persuadables，并显式排除 Sleeping Dogs**；随后在统一成本口径下评估这一选定策略的 ROI proxy。预算扫描仅作为未来若需要 score-based 扩量时的补充上界分析。

在 `notebooks/05_segmentation_and_roi.ipynb` 的可核验输出中，`qini_results.json` 指向的最佳 learner 为 X-Learner（held-out `Qini Coef = 1.72`）；Phase 3 随后从 `cate_vectors.npz` 读取对应的全样本 CATE 向量，其 `shape = (64000,)`，`range = [-0.049814, 0.045046]`。
...
作为补充的 budget-sweep / upper-bound 分析，结果进一步显示：`Precision reaches ≥95% of full uplift at budget ≈ 60%`（见同一 Notebook 的输出与 `outputs/figures/fig_08b_budget_uplift_curve.png`）。该结果用于回答“若未来按连续 score 扩量，何时接近 full uplift”，不代表当前选定策略。
...
Phase 3 的范围聚焦在“从 CATE 向量到离线策略建议”的翻译层，覆盖三块内容：
...
3) 选定的 `Persuadables only` 策略评估，以及作为补充扩量分析的预算扫描（Full vs Random vs Persuadables-only；Budget vs Cumulative uplift），实现/证据入口：`src/business.py`、`notebooks/05_segmentation_and_roi.ipynb`、`outputs/figures/fig_08*.png`。
...
- Optional persisted output (local rerun; gitignored):
  - User-level segment artifact for local SQL demo: `data/processed/user_segments.csv`
  - ROI simulation results: `data/processed/roi_simulation.json`

## 5. End-to-End Pipeline Sketch

下面用真实产物把 Phase 3 的端到端链路串起来：先读取 `qini_results.json` 中 held-out Qini 选出的 `best_learner`，再从 `cate_vectors.npz` 中提取对应的全样本 CATE 向量，把这条上游 handoff 翻译为四象限分群，并导出 repo-local 的 `user_segments.csv` 供本地 SQL demo 使用；随后用分群结果评估选定的 `Persuadables only` 策略，并额外运行基于连续 CATE 排序的预算扫描，作为未来扩量场景的补充分析，最终把图表与 JSON 落盘以便审阅。

```mermaid
flowchart LR
  A[Feature table<br/>data/processed/hillstrom_features.csv] --> D[Load and validate Phase 2 handoff artifacts<br/>notebooks/05_segmentation_and_roi.ipynb]
  B[qini_results.json<br/>held-out eval + best_learner] --> D
  C[cate_vectors.npz<br/>full-sample cate_s/t/x] --> D
  D --> E[Resolve Phase 2-selected CATE vector<br/>current artifacts: X-Learner]
  E --> F[Quadrant segmentation<br/>src/business.segment_users]
  F --> F0[User-level segments<br/>data/processed/user_segments.csv]
  F --> F1[Figures<br/>outputs/figures/fig_07*.png]
  F --> G[ROI simulation<br/>src/business.simulate_roi]
  G --> G1[Figures<br/>outputs/figures/fig_08*.png]
  G --> G2[roi_simulation.json<br/>data/processed/roi_simulation.json]
```
...
1) 打开 `notebooks/05_segmentation_and_roi.ipynb`，在 stdout/表格输出中核验：
    - `Phase 2 handoff: qini_results.json reports best_learner = X-Learner (Qini Coef = 1.72)` 与 `Loaded corresponding full-sample CATE vector for the Phase 2-selected learner from cate_vectors.npz: shape = (64000,), range = [-0.049814, 0.045046]`。
    - 分群 summary 表（四象限的 Count/Pct/Mean CATE/Mean Baseline）及敏感性分析表（P30-P70）。
    - 选定策略 ROI stdout：`ROI = 0.004285` vs `ROI = 0.008893`、`节省预算 75.0%`、`保留增量 51.9%`；这些数字对应 `Persuadables only` 的离线策略模拟结果。
    - 补充预算扫描 stdout：`Precision reaches ≥95% of full uplift at budget ≈ 60%`；该结果仅用于说明连续 CATE 排序下的扩量上界 / sensitivity，不代表当前选定策略。
    - 若本地已复跑最新版本，还可核验 `user_segments.csv` 的导出提示与导出后 schema/行数检查；该文件在本项目里只作为 local SQL demo artifact，不应解读为生产 activation feed。
...
### 7.3 Selected Policy Evaluation (Full vs Persuadables-only)
...
此外，作为补充的预算扫描曲线，stdout 打印：`Precision reaches ≥95% of full uplift at budget ≈ 60%`，并落盘到 `outputs/figures/fig_08b_budget_uplift_curve.png`。这反映的是“若未来按连续 CATE 排序继续扩量”的上界参考，而不是当前默认的 `Persuadables only` 策略。

## 8. Method Notes

### 8.1 Quadrant Definitions Used Here

本阶段的“四象限”是一组策略分层规则：它把连续 CATE 向量（边际增量）离散成四个便于离线比较的标签，并附带一个 baseline 的 proxy 轴，用于区分“高增量但基线更低/更高”。实现见 `src/business.py`。
...
Phase 3 的输出应被解读为“基于离线 CATE 排序、但最终落到 `Persuadables only` 默认策略上的策略层结果”，其边界包括：
...
Phase 3 的核心工作，是把 Phase 2 的 CATE 从“模型输出”翻译成“离线策略建议”。

具体而言，本阶段先读取 Phase 2 在 Notebook 04 中已选优并落盘的 learner 元数据与全样本 CATE 向量；在当前持久化结果里，held-out Qini 最优 learner 为 X-Learner，因此 Phase 3 实际消费的是对应的全样本 CATE（`n=64,000`，range `[-0.049814, 0.045046]`），再通过 `src/business.segment_users` 将人群切分为四象限。敏感性分析进一步表明，默认阈值（`baseline_threshold=P50`）能够得到稳定且可解释的分群结构。

随后，`src/business.simulate_roi` 在统一成本口径下对比“全量投放 vs 随机投放 vs 精准投放（Persuadables only）”，得到一组可核验的 offline policy simulation 结论：仅投放 `25%` 的 Persuadables，预计可节省 `75%` 预算，同时预计保留 `51.9%` 的增量转化，`ROI proxy ratio` 为 `2.08×`。另一个补充发现是：若未来按连续 score 做扩量，预算扫描曲线显示在约 `60%` 预算时即可获得 `≥95%` 的全量增量效果。

### 11.2 Structured Recap

- Context：输入为 Phase 2 已落盘的全样本 CATE 向量与 `best_learner` 元数据，目标是将模型输出转成可复现的离线排序规则。
- Objective：构建稳定、可解释的四象限分群，并量化选定的 `Persuadables only` 策略相对 Full / Random 的 offline ROI proxy 差异；budget-sweep 仅作补充扩量分析。
- Outcome：Persuadables 占 `25%`；选定策略的 `ROI proxy` 为 `0.008893`，相对 Full 的 `0.004285` 比值为 `2.08×`。另一个补充上界结果是：若未来按连续 score 扩量，在约 `60%` 预算时可覆盖 `≥95%` 的全量增量效果。
